...
class ControlModuleEnv(Env):
    def __init__(self,GUI_flag=False):
        ...
        self.identi_flag=False
        self.go_back_flag=False
        self.addition_grip_flag=False
        self.already_identi_flag=False
        ...
        self.block_id = pb.loadURDF("/home/tianxu/Documents/DMP-python/Dual_Arm_New/Robot_Sim/urdf/object/magnetic.urdf",self.block_start_position_random
                               , block_start_orientation)
        self.table_id = pb.loadURDF("/home/tianxu/Documents/DMP-python/Dual_Arm_New/Robot_Sim/urdf/object/table.urdf",
        ...
        self.robot1._setRobotiqPosition(0.12)
        reset_dmp()

        num_joints = pb.getNumJoints(self.robot1.id)
        self.controlled_joints = [i for i in range(num_joints) if
                                  pb.getJointInfo(self.robot1.id, i)[2] != pb.JOINT_FIXED]
        self.arm_joint_indices = [1, 2, 3, 4, 5, 6, 7]

        #

    def mass_identi_object(self):
        print("start mass identification")
        joint_angle1 = self.robot1._calculateIK(np.array(self.identi_tra[self.mass_identi_step]) + [0.0, 0.0, -0.0], rotation_ee)
        ...
        js = pb.getJointStates(self.robot1.id, self.controlled_joints)
        q = np.array([s[0] for s in js])
        q_dot = np.array([s[1] for s in js])
        tau = np.array([s[3] for s in js])

        js = pb.getJointStates(self.robot1.id, self.arm_joint_indices)
    ...
    def go_back_process(self):
        ...
        if np.linalg.norm(dist_target_) < step_dist_return:
            ...
            flag=True
            print("get the final position",base_position_grip)
    ...
    def go_catch_object(self,target_position):
        print("reach_step", self.reach_step, "close_step", self.close_step,"target",target_position)
        finish_flag = False
    ...
    def step(self,action):
        ...
        obs = np.concatenate((np.array([self.mass]),np.array([self.friction]),np.array([action[0]]),end_effector_pos1, end_effector_ori1,joint_angle_m, np.array([dmps[key].cs.s]),
        ...
        if self.all_step_count < initial_step_num:
        ...
        elif not (self.initial_catch_flag or self.identi_flag or self.already_identi_flag) :
            if self.all_step_count == initial_step_num:
            ...
            if step_finish_flag == True:
                ...
                self.base_position = end_effector_pos1_modified.copy()
                end_effector_pos1_modified += np.array([0, 0, 0.02])
                # preliminary save this position to dmp initial position

                print("end_effector_pos1_modified", end_effector_pos1_modified)
                lift_pos = end_effector_pos1_modified + np.array([0, 0, 0.1])

                # self.initial_catch_flag = True
                tra_pre = np.array(
                    [np.linspace(end_effector_pos1_modified[i], end_effector_pos1_modified[i], 5) for i in range(3)]).T
                tra_go = np.array([np.linspace(end_effector_pos1_modified[i], lift_pos[i], 160) for i in range(3)]).T
                self.identi_tra = np.vstack((tra_pre, tra_go))

                self.identi_flag = True
                self.init_global_params()
        ...
        if self.identi_flag:
            identi_finish_flag = self.mass_identi_object()
            self.mass_identi_step += 1
            if identi_finish_flag==1:
                print("enter slip out process")

                self.identi_flag = False
                self.addition_grip_flag=True
                self.already_identi_flag=True
            if identi_finish_flag==2:
                print("enter grip process")
        ...
        if self.go_back_flag:
            print("start go back")
            goback_finish_flag = self.go_back_process()

            time.sleep(sleep_time_set)
            if goback_finish_flag:
                print("go back finished")
                self.go_back_flag = False
                self.addition_grip_flag = True


        if self.addition_grip_flag:
            print("start additional grip")
            base_position, base_orientation = pb.getBasePositionAndOrientation(self.block_id)
            base_position = np.array(base_position)
            base_position[2] = end_effector_zbias
            self.base_position = base_position
        ...
        if self.initial_catch_flag:
            ...
            obs = np.concatenate(
                (np.array([self.mass]),np.array([self.friction]),np.array([action[0]]), end_effector_pos1, end_effector_ori1, joint_angle_m, np.array([dmps[key].cs.s]),
                 np.array(self.block_start_position_random), np.array(self.block_target_position_random)))
    ...
    def reset(self):
        ...
        self.block_id = pb.loadURDF(
            "/home/tianxu/Documents/DMP-python/Dual_Arm_New/Robot_Sim/urdf/object/magnetic.urdf",
        ...
        # Set friction for object1
        pb.changeDynamics(self.block_id, -1, lateralFriction=1)
        pb.changeDynamics(self.block_id, -1, rollingFriction=0.01, spinningFriction=0.01)
        pb.changeDynamics(self.block_id, linkIndex=-1, mass=self.mass)  # Change the mass of the first link to 5 kg
        # pb.changeDynamics(block_id, linkIndex=-1, mass=10)  # Change the mass of the first link to 5 kg
        ...
        obs = np.concatenate((np.array([self.mass]),np.array([self.friction]),np.array([0.0]),end_effector_pos1,end_effector_ori1, joint_angle_m, np.array([dmps[key].cs.s]),np.array(self.block_start_position_random), np.array(self.block_target_position_random)))
