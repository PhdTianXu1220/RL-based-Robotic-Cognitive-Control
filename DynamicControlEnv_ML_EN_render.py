...
class ControlModuleEnv(Env):
    ...
    def go_catch_object(self,target_position):
        ...
        if (self.reach_step > grip_open_steps) and (dist_target_ < close_thres):
            ...
            # self.end_effector_pos1 = base_position_grip.tolist()
            # self.end_effector_ori1 = rotation_ee
        ...
        if self.reach_step <= grip_open_steps:
            ...
            if np.linalg.norm(dist_target_) < step_dist_return:
                ...
                # self.end_effector_pos1 = next_pos.tolist()
                # self.end_effector_ori1 = rotation_ee
    ...
    def step(self,action):
        ...
        if self.all_step_count < initial_step_num:
            ...
            end_effector_pos1 = np.array(l[0])
            end_effector_ori1 = np.array(l[1])
            self.end_effector_pos1=end_effector_pos1.tolist()
            self.end_effector_ori1=end_effector_ori1.tolist()
        ...
        if self.initial_catch_flag:
            ...
            print("manipulator torque,",np.abs(manipulator_torque) ,"judge",torque_flag,"time",self.execution_time)
    ...
    def reset(self):
        ...
        goal_random_array = np.random.uniform(low=goal_low_bounds, high=goal_high_bounds)
        target_bias = goal_random_array
        target_bias=np.array([-0.4, 0.1, 0])

        mass_low_bounds = 0.15
        ...
        # self.block_start_position_random = block_start_position + table_bias
        # self.block_target_position_random = np.array(
        #     [0.28667097385077456, -0.48847984909271797, 0.5940973561794143]) + table_bias
