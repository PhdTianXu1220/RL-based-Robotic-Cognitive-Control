...
from utils_PPO import str2bool, Action_adapter, Reward_adapter, evaluate_policy, evaluate_policy_new,collect_data
from PPO import PPO_agent

from DynamicControlEnv_ML_EN_render import ControlModuleEnv
import numpy as np
...
def main():
    ...
    if opt.render:
        while True:
            ...
            ep_r, success_rate, mean_time, mean_energy = evaluate_policy_new(judge_model,judge_device,env, agent, opt.max_action, turns=100)
            print(f'Env:{EnvName[opt.EnvIdex]}, Episode Reward:{ep_r}, Success Rate:{success_rate}, Mean Time:{mean_time}, Mean Energy:{mean_energy}')

            # function for collecting data
            # observations, actions, rewards=collect_data(env, agent, opt.max_action, turns=1000)
            # data_dir = 'JudgeModuleData'
            # np.save(os.path.join(data_dir, 'observations20250615.npy'), observations)
            # np.save(os.path.join(data_dir, 'actions20250615.npy'), actions)
            # np.save(os.path.join(data_dir, 'rewards20250615.npy'), rewards)
