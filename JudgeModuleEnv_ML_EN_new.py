...
end_effector_zbias=0.5940973561794143 + 0.022+table_bias[2]+0.01
...
class JudgeModuleEnv(Env):
    def __init__(self,GUI_flag=False):
        super(JudgeModuleEnv, self).__init__()
        self.reach_step=0
        self.close_step=0
        ...
        self.controlled_joints = [i for i in range(num_joints) if pb.getJointInfo(self.robot1.id, i)[2] != pb.JOINT_FIXED]
    ...
    def step(self,action):
        ...
        if self.all_step_count>=max_step:
            ...
            return self.env_end_flag,self.task_success
        ...
        if self.all_step_count < initial_step_num:
        ...
        elif not (self.initial_catch_flag or self.identi_flag) :
            if self.all_step_count == initial_step_num:
            ...
            if step_finish_flag == True:
                ...
                self.base_position = end_effector_pos1_modified
                ...
                tra_pre = np.array([np.linspace(end_effector_pos1_modified[i], end_effector_pos1_modified[i], 5) for i in range(3)]).T
        ...
        if self.initial_catch_flag:
            ...
            for k in range(dmp_step_num):
                ...
                # else:
                x_track_s, _, _ = dmps[key].step(tau=2)
