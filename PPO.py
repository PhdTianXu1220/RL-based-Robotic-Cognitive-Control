from utils_PPO import BetaActor, GaussianActor_musigma, GaussianActor_mu, Critic
import numpy as np
import copy
import torch
import math
...
class PPO_agent(object):
	...
	def save(self,timestep):
		torch.save(self.actor.state_dict(), "./ppo_model/20240606/PPO_actor{}.pth".format(timestep))
		torch.save(self.critic.state_dict(), "./ppo_model/20240606/PPO_q_critic{}.pth".format(timestep))

	def load(self, timestep):
		self.actor.load_state_dict(torch.load("./ppo_model/20240606/PPO_actor{}.pth".format(timestep), map_location=self.dvc))
		self.critic.load_state_dict(torch.load("./ppo_model/20240606/PPO_q_critic{}.pth".format(timestep), map_location=self.dvc))
