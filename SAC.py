from utils_SAC import Actor, Double_Q_Critic
...
class SAC_countinuous():
	...
	def train(self,):
		...
		q_loss.backward()
		torch.nn.utils.clip_grad_norm_(self.q_critic.parameters(), max_norm=1.0)
		...
		if self.adaptive_alpha:
			...
			alpha_loss.backward()
			# Clip gradients of the `log_alpha` parameter to avoid large updates
			torch.nn.utils.clip_grad_norm_([self.log_alpha], max_norm=1.0)

			self.alpha_optim.step()
			self.alpha = self.log_alpha.exp().clamp(min=1e-3, max=5.0)
	...
	def save(self,timestep):
		#20241221 save the success results for cross simulation
		#f"./sac_model/20241228/sac_actor_{timestep}.pth" save the success results for physic cross experiments
		torch.save(self.actor.state_dict(), f"./sac_model/20250612/sac_actor_{timestep}.pth")
		torch.save(self.q_critic.state_dict(), f"./sac_model/20250612/sac_critic_{timestep}.pth")

	def load(self,timestep):
		self.actor.load_state_dict(torch.load(f"./sac_model/20250612/sac_actor_{timestep}.pth", map_location=self.dvc,weights_only=True))
		self.q_critic.load_state_dict(torch.load(f"./sac_model/20250612/sac_critic_{timestep}.pth", map_location=self.dvc,weights_only=True))
